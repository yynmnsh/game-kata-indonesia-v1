...
class APIWordService {
    constructor() {
        ...
        this.apis = {
            kateglo: 'http://kateglo.lostfocus.org/api.php',
            kbbi: 'https://new-kbbi-api.herokuapp.com',
            wordlist: 'https://raw.githubusercontent.com/andiazhar/indonesia-word-list/master/indonesia-word-list.txt'
        };
        this.initCache();
    }
    ...
    async fetchBulkWordList() {
        try {
            ...
            const response = await fetch(this.apis.wordlist);
    ...
    /**
     * Validate a word using Kateglo API
     */
    async validateWord(word) {
        ...
        try {
            // Try Kateglo API
            const url = `${this.apis.kateglo}?format=json&phrase=${encodeURIComponent(word)}`;
            const response = await fetch(url);
            
            if (!response.ok) throw new Error('Kateglo API failed');
            
            const data = await response.json();
            const isValid = data.kateglo && data.kateglo.definition;
            ...
            console.warn('Kateglo API failed, trying KBBI API...', error);
            
            try {
                // Fallback to KBBI API
                const url = `${this.apis.kbbi}/cari/${encodeURIComponent(word)}`;
                const response = await fetch(url);
                
                if (!response.ok) throw new Error('KBBI API failed');
                ...
                return await this.checkInCachedList(word);
            }
        }
    }
